...
def commit_changes(issue, branch) -> None:
    ...
    commit_message = (
        ...
        f"Automated fix by clanker Agent.\n"
...
def build_and_create_pr(
    ...
    pr_body = (
        ...
        f"**Issue:** {issue.title}\n\n"
        f"### Changes\n"
        f"This PR was automatically generated by clanker Agent to resolve #{issue.number}.\n\n"
        f"### Test Status\n"
        f"{test_status} (coding attempts: {coding_attempts})\n\n"
        f"{cost_section}"
        f"{screenshots_md}\n\n"
        f"---\n"
        f"*Generated by clanker Autofix Agent*"
    )
...
def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("clanker Agent starting")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info("clanker Agent complete")
    logger.info("=" * 60)
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"clanker Agent failed: {e}", exc_info=True)
        sys.exit(1)
